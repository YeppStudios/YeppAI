...
const plans = [
  {
    ...
    features: [
      "200 000 elixir/mo (~100 A4 pages)",
      "1 folder for assets",
      "10 assets to upload (max 15MB total)",
      "3 teammates",
      "Single web pages scanning",
      "Upload PDF, PPTX, TXT, CSV and DOCX",
      "Chat with your AI",
      "Marketing templates",
      "Copywriting feature",
    ],
    ...
    polishPrice: 299,
    monthlyPriceId: {
      default: "price_1NaF8EFe80Kn2YGGAuVBGHjh",
      polish: "price_1NUPqFFe80Kn2YGG0FZyaRXH",
    },
    threeMonthPriceId: {
      default: "price_1NaFKUFe80Kn2YGGN8gOfDnT",
      polish: "price_1NUPqQFe80Kn2YGGAjjfYZXQ",
    },
    sixMonthPriceId: {
      default: "price_1NaFLfFe80Kn2YGGFtgjV1CI",
      polish: "price_1NUPqcFe80Kn2YGG5zUJqhKq",
    },
    yearlyPriceId: {
      default: "price_1NaFMhFe80Kn2YGGsXAeqFPF",
      polish: "price_1NUPr0Fe80Kn2YGGd6BuzOAs",
    },
  },
  {
    title: "Agency",
    ...
    features: [
      "1M elixir/mo (~520 A4 pages)",
      "Unlimited folders for assets",
      "Unlimited number of assets",
      "Unlimited storage space",
      "Unlimited number of teammates",
      "Content Campaign feature",
    ...
    polishPrice: 799,
    monthlyPriceId: {
      default: "price_1NSZghFe80Kn2YGGOiClJUPM",
      polish: "price_1NUPofFe80Kn2YGG6dYxHNk9",
    },
    threeMonthPriceId: {
      default: "price_1NSai5Fe80Kn2YGGHrwmUEqe",
      polish: "price_1NUPozFe80Kn2YGGComghBF5",
    },
    sixMonthPriceId: {
      default: "price_1NSaiNFe80Kn2YGGG88egvhI",
      polish: "price_1NUPpBFe80Kn2YGGW0muvINv",
    },
    yearlyPriceId: {
      default: "price_1NSaieFe80Kn2YGGilwS3SNl",
      polish: "price_1NY1QdFe80Kn2YGGaQBjxlGP",
    },
  },
  {
    title: "Custom",
...
const Plans = (props: {
  ...
  const renderPlans = () => {
    const renderedPlans = plans.map((plan) => {
      ...
      if (billingPeriod === 1) {
        country === "Poland"
          ? (priceId = plan.monthlyPriceId.polish)
          : (priceId = plan.monthlyPriceId.default);
      } else if (billingPeriod === 3) {
        country === "Poland"
          ? (priceId = plan.threeMonthPriceId.polish)
          : (priceId = plan.threeMonthPriceId.default);
      } else if (billingPeriod === 6) {
        country === "Poland"
          ? (priceId = plan.sixMonthPriceId.polish)
          : (priceId = plan.sixMonthPriceId.default);
      } else if (billingPeriod === 12) {
        country === "Poland"
          ? (priceId = plan.yearlyPriceId.polish)
          : (priceId = plan.yearlyPriceId.default);
      }
      ...
      return (
        <MainPlanContainer key={plan.title}>
          {plan.title === "Standard" && (
            <PlanContainer backgroundColor="white" color="black" width="auto">
              <PlanTitle>
                ...
                <PlanTitleText>{plan.title}</PlanTitleText>{" "}
              </PlanTitle>
              <BriefDescription>Best for a freelancer</BriefDescription>
              <MainDescription>
                Your personal AI content creator with knowledge from uploaded
                assets.
              </MainDescription>
              ...
              {country !== "Poland" ? (
                  ...
                  {billingPeriod === 1 ? (
                    ...
                    <Price>
                      ...
                      {(
                        ...
                        billingPeriod * plan.price * discount
                      ).toFixed(0)}
                      <Monthly>/{billingPeriod}mo</Monthly>
                      <Gross>
                        <ColorfulText>
                          you save $
                          {(billingPeriod * plan.price * discount).toFixed(0)}
                        </ColorfulText>
                      </Gross>
                    </Price>
                  )}
                ...
                <Price>
                  {billingPeriod === 1 ? (
                    <Price>
                      {plan.polishPrice}zł<Monthly>/mo</Monthly>
                    </Price>
                  ) : (
                    <Price>
                      {(
                        plan.polishPrice * billingPeriod -
                        billingPeriod * plan.polishPrice * discount
                      ).toFixed(0)}
                      zł<Monthly>/{billingPeriod}mo</Monthly>
                      <Gross>
                        <ColorfulText>
                          you save{" "}
                          {(
              ...
              <Centered>
                <Note>
                  No pressure. You can change plans or cancel anytime.
                </Note>
              </Centered>
              <Centered>
                {props.openRegistration ? (
                  <BuyButton
                    id="standard-free-trial-btn"
                    onClick={() =>
                      router.push(
                        `/register?registration=true&priceId=${priceId}&planName=${plan.title}&planId=${plan.planId}&billingPeriod=${billingPeriod}`
                      )
                    }
                    backgroundColor="black"
                    color="white"
                  >
                    {loadingBtn === plan.title ? (
                      <Loader color="white" />
                    ) : (
                      <>
                        <BtnIcon>
                          <BiGift style={{ width: "100%", height: "auto" }} />
                        </BtnIcon>
                        {props.landing ? (
                          <p>Buy now</p>
                        ) : (
                  ...
                  <>
                    <BuyButton
                      id="standard-buy-now-btn"
                      onClick={() =>
                        openCheckout(priceId, plan.title, plan.planId)
                      }
                      backgroundColor="black"
                      color="white"
                    >
          ...
          {plan.title === "Agency" && (
            ...
            >
              <PlanTitle>
                ...
                <PlanTitleText>
                  <ColorfulText>{plan.title}</ColorfulText>
                </PlanTitleText>{" "}
              </PlanTitle>
              <BriefDescription>Best for a marketing agency</BriefDescription>
              <MainDescription>
                Unleash the full potential of the platform and maximize your
                performance.
              </MainDescription>
              {country === "Poland" && <PriceLabel>Net price:</PriceLabel>}
              {country !== "Poland" ? (
              ...
              <FeaturesList>
                ...
                <Feature>
                  ...
                  <FeatureText>
                    <ColorfulText>
                      <b>Everything from Standard plan</b>
                    </ColorfulText>
                  </FeatureText>
                </Feature>
              </FeaturesList>
          ...
          {plan.title === "Custom" && (
            <PlanContainer
              backgroundColor="rgba(101, 120, 248, 0.2)"
              color="black"
              width="auto"
            >
              <PlanTitle>
              ...
              <MainDescription>
                Take advantage of individually prepared folders and API access.
              </MainDescription>
              {country === "Poland" && <PriceLabel>Price:</PriceLabel>}
              <Price>Individual</Price>
              <Centered>
                <Note>
                  pitch us your idea and we will come up with a tailored plan
                </Note>
              </Centered>
              <Centered>
                <BuyButton
    ...
    return <PlansContainer>{renderedPlans}</PlansContainer>;
  };

  return (
    <>
      {openLoginModal && (
        <LoginModal onClose={undefined} registration={false} />
      )}
      {openContact && (
        <PhoneNumberPopup onClose={() => setOpenContact(false)} />
      )}
      <div className="w-full flex justify-center mt-8">
        <div className="sm:hidden">
          ...
          >
            {tabs.map((tab) => (
              <>
                <option value={tab.period} key={tab.name}>
                  {tab.name}
                </option>
              </>
            ))}
          ...
          >
            {tabs.map((tab) => (
              <div
                ...
                key={tab.name}
                onClick={() => setBillingPeriod(tab.period)}
              >
                <button
                  className={classNames(
                    tab.period === billingPeriod
                      ? "bg-blue-100 py-4 text-blue-700"
                      : "text-gray-500 hover:text-gray-700",
                    "rounded-lg px-14 text-lg font-medium"
                  )}
                >
                  {tab.period === billingPeriod ? (
                    <ColorfulText>{tab.name}</ColorfulText>
                  ) : (
                    tab.name
                  )}
                </button>
                {tab.period !== billingPeriod && tab.period !== 1 && (
                  <div className="text-gray-500">
                    <ColorfulText>
                      <b>{tab.discount * 100}% off</b>
                    </ColorfulText>
                  </div>
                )}
              </div>
