...
const IdeaCreator = ({back, query, template}: any) => {
  ...
  const generateContent = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (query.page === "hashtags") {
      ...
      setPrompt(`Please as Google Keywords Planner come up with ${numberOfIdeas} excellent keyword ideas for ${idea}. Ensure each keyword matches the topic, has high chance of good conversion, has a good search/competition ratio and will rank well on Google. Make sure it is worth the ${targetAudience} attention. Make sure the keywords are written correctly in ${language} language. Respond with numbered list of keywords. Make sure they are no longer than 30 characters and strictly follow Google Ads guidelines. Do not quote them.`)
    } else {
